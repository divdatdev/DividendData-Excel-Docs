...
> **Jump to:** [Installation](#installation) · [Quick Action Sidebar](#quick-action-sidebar) · [Function Builder](#function-builder) · [Function Reference](#function-reference)
...
Once installed, you have two powerful tools available from the **Dividend Data** tab in your Excel ribbon:

1. **Open Sidebar** — Load full data tables into your sheet with one click (no formulas needed).
2. **Function Builder** — Build any formula visually and copy it into your cells.
...
1. Click the **Dividend Data** tab in your Excel ribbon (top of the window).
2. Click **Open Sidebar** (the first button on the left).
3. The sidebar panel opens on the right side of your spreadsheet.

> **Tip:** If you don't see the Dividend Data tab, the add-in may not be loaded yet. Go to **Home → Add-ins** and make sure Dividend Data is enabled.

### Signing In

When the sidebar opens, you need to sign in with the email address associated with your Dividend Data account.

1. Click the **Sign In** button at the top of the sidebar.
2. Enter your email address and confirm.
3. A green status bar will confirm you're signed in (e.g., "Signed in as: you@example.com").

If you don't have an account yet, click **Sign Up** at the bottom of the sidebar to create a free account at DividendData.com.

### Step 1 — Find a Stock or Fund

At the top of the sidebar, you'll see a search bar labeled **"Search stocks (symbols, names)"**.
...
- Once selected, the sidebar shows example formulas you can click to paste directly into a cell. This is useful if you want a quick single data point rather than a full table.

### Step 2 — Load Data into Your Sheet

Below the search bar, you'll see the **Quick Action** buttons arranged in a grid:
...
- **Refresh** — Reload the sidebar if anything looks off.
- **Sign Out** — Switch accounts or sign out when done.
...
| You're new and want to see what the add-in offers | You know the exact metric you need |
...
The Function Builder is a visual formula editor built into the add-in. It walks you through every data category, every available metric, and every option — then generates the exact formula for you. No memorization, no guessing syntax. Just configure and copy.
...
**From the Ribbon:**
1. Click the **Dividend Data** tab in your Excel ribbon.
2. Click **Function Builder** (the `fx` icon, third button in the toolbar).

**From the Sidebar:**
1. If the sidebar is already open, click the **Function Builder** link in the tips section, or close the sidebar and use the ribbon button above.

> To return to the main sidebar from the Function Builder, click **← Back to Main** at the top-left of the Function Builder panel.
...
|----------|----------|-------------|
| Dividends | `DIVIDENDDATA.DIVIDENDS` | Payouts, yields, history, growth rates, payout ratios |
| Financials | `DIVIDENDDATA.STATEMENT` | Income Statement, Balance Sheet, Cash Flow |
| Metrics | `DIVIDENDDATA.METRICS` | 100+ individual metrics (Revenue, EPS, FCF, etc.) |
| Ratios | `DIVIDENDDATA.RATIOS` | 50+ financial ratios (P/E, ROE, D/E, etc.) |
| Growth | `DIVIDENDDATA.GROWTH` | Growth rates (revenue growth, EPS growth, etc.) |
| Quotes | `DIVIDENDDATA.QUOTE` | Live price, volume, moving averages, price history |
| Profile | `DIVIDENDDATA.PROFILE` | Company info — sector, industry, CEO, description |
| Funds | `DIVIDENDDATA.FUND` | ETF/fund holdings, expense ratio, sector allocation |
| Estimates | `DIVIDENDDATA.ESTIMATES` | Analyst EPS and revenue forecasts |
| Segments | `DIVIDENDDATA.SEGMENTS` | Product and geographic revenue breakdowns |
| KPIs | `DIVIDENDDATA.KPIS` | Company-specific operational metrics |
| Crypto | `DIVIDENDDATA.CRYPTO` | Cryptocurrency prices and history |
| Commodities | `DIVIDENDDATA.COMMODITIES` | Commodity prices and history (oil, gold, etc.) |
| Price Targets | `DIVIDENDDATA.PRICE_TARGET` | Analyst consensus, high, low, median targets |
...
- **Period** — `Annual` or `Quarterly` (where applicable).
...
At the bottom of the Function Builder, you'll see the **live formula preview** — a code box that shows the exact Excel formula based on your current selections.

**Example preview:**
```
=DIVIDENDDATA.DIVIDENDS("AAPL", "summary", TRUE)
```
